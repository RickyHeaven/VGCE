<script setup lang="ts">
  ...
  import {
    getCenterPoint, randomString, getSvgNowPosition, setSvgActualInfo, prosToVBind, objectDeepClone, getCommonClass
  } from '@/utils'
  import {
    calculateBottom,
    calculateLeft,
  ...
  const cursor_style = computed(() => globalStore.intention == EGlobalStoreIntention.MoveCanvas ? 'grab' :
    globalStore.intention == EGlobalStoreIntention.Rotate ? "url('/src/assets/icons/rotate.svg') 12 12, auto" :
      'default')
  const visible_info: IVisiableInfo = reactive({
    handle_panel: computed(() => globalStore.intention === EGlobalStoreIntention.Select || globalStore.intention ===
      EGlobalStoreIntention.Zoom || globalStore.intention === EGlobalStoreIntention.Rotate),
  ...
  const onSvgMouseEnter = (select_item: IDoneJson, index: number, e: MouseEvent) => {
    ...
    if ((globalStore.intention === EGlobalStoreIntention.Connection || globalStore.intention ===
      EGlobalStoreIntention.SetConnectionLineNode) && select_item.type === EDoneJsonType.ConnectionLine) {
      return
    }
  ...
  const onCanvasMouseMove = (e: MouseEvent) => {
    ...
    if (globalStore.handle_svg_info?.info &&
      ...
      if (globalStore.scale_info.type === EScaleInfoType.TopLeft) {
        new_length =
          calculateLeftTop(curPosition, globalStore.scale_info.symmetric_point, globalStore.handle_svg_info.info.rotate)
      }
      else if (globalStore.scale_info.type === EScaleInfoType.TopRight) {
        new_length = calculateRightTop(curPosition, globalStore.scale_info.symmetric_point,
          globalStore.handle_svg_info.info.rotate)
      }
      else if (globalStore.scale_info.type === EScaleInfoType.BottomRight) {
        new_length = calculateRightBottom(curPosition, globalStore.scale_info.symmetric_point,
          globalStore.handle_svg_info.info.rotate)
      }
      else if (globalStore.scale_info.type === EScaleInfoType.BottomLeft) {
        new_length = calculateLeftBottom(curPosition, globalStore.scale_info.symmetric_point,
          globalStore.handle_svg_info.info.rotate)
      }
      else if (globalStore.scale_info.type === EScaleInfoType.TopCenter) {
        new_length =
          calculateTop(curPosition, globalStore.scale_info.symmetric_point, globalStore.handle_svg_info.info.rotate,
            globalStore.handle_svg_info.info.client)
      }
      else if (globalStore.scale_info.type === EScaleInfoType.Right) {
        new_length =
          calculateRight(curPosition, globalStore.scale_info.symmetric_point, globalStore.handle_svg_info.info.rotate,
            globalStore.handle_svg_info.info.client)
      }
      else if (globalStore.scale_info.type === EScaleInfoType.BottomCenter) {
        new_length =
          calculateBottom(curPosition, globalStore.scale_info.symmetric_point, globalStore.handle_svg_info.info.rotate,
            globalStore.handle_svg_info.info.client)
      }
      else if (globalStore.scale_info.type === EScaleInfoType.Left) {
        new_length =
          calculateLeft(curPosition, globalStore.scale_info.symmetric_point, globalStore.handle_svg_info.info.rotate,
            globalStore.handle_svg_info.info.client)
      }
      ...
      if (globalStore.handle_svg_info && new_length.width > 0 && new_length.height > 0) {
        const scale_x = !new_length.is_old_width ?
          new_length.width / globalStore.handle_svg_info.info.actual_bound.width : 1
        const scale_y = !new_length.is_old_height ?
          new_length.height / globalStore.handle_svg_info.info.actual_bound.height : 1
        const newCenterPoint = getCenterPoint(curPosition, globalStore.scale_info.symmetric_point)
        if (scale_x > 0 && globalStore.scale_info.type !== EScaleInfoType.TopCenter && globalStore.scale_info.type !==
          EScaleInfoType.BottomCenter) {
          globalStore.handle_svg_info.info.scale_x = scale_x
          globalStore.handle_svg_info.info.x =
            getSvgNowPosition(globalStore.handle_svg_info.info.client.x, newCenterPoint.x,
              globalStore.scale_info.scale_item_info.x)
        }
        if (scale_y > 0 && globalStore.scale_info.type !== EScaleInfoType.Left && globalStore.scale_info.type !==
          EScaleInfoType.Right) {
          globalStore.handle_svg_info.info.scale_y = scale_y
          globalStore.handle_svg_info.info.y =
            getSvgNowPosition(globalStore.handle_svg_info.info.client.y, newCenterPoint.y,
      ...
      const rotateDegreeBefore = Math.atan2(
        globalStore.mouse_info.position_y - globalStore.handle_svg_info.info.client.y,
        globalStore.mouse_info.position_x - globalStore.handle_svg_info.info.client.x) / (Math.PI / 180)
      const rotateDegreeAfter = Math.atan2(
        clientY - svgEditLayoutStore.center_offset.y - globalStore.handle_svg_info.info.client.y,
        clientX - svgEditLayoutStore.center_offset.x - globalStore.handle_svg_info.info.client.x) / (Math.PI / 180)
      ...
      globalStore.handle_svg_info.info.props.point_position.val[globalStore.handle_svg_info?.info.props.point_position.val.length -
      1] = {
        x: getSvgNowPosition(globalStore.mouse_info.position_x, clientX,
          globalStore.handle_svg_info?.info.props.point_position.val[0].x),
        y: getSvgNowPosition(globalStore.mouse_info.position_y, clientY,
  ...
  const onSvgContextMenuEvent = (select_item: IDoneJson, index: number, e: MouseEvent) => {
    ...
    if (contextMenuRef.value) {
      ...
      contextMenuStore.info.MoveUpOneLevel.enable = contextMenuStore.info.MoveUpTopLevel.enable =
        contextMenuStore.info.MoveDownOneLevel.enable = contextMenuStore.info.MoveDownTopLevel.enable = true
